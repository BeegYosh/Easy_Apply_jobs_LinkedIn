...
If you see "Failed to calculate the date posted" in logs, LinkedIn may include the job location in the same line as the posted time. The tool now extracts the first time-like expression from that string, so updating should resolve this.
