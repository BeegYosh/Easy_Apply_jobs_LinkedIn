...
## ❓ Troubleshooting
If you encounter a "Failed to find Job listings! In Applier" message, make sure you are using an up-to-date version. The bot now searches for `li[data-occludable-job-id]` elements and falls back to the generic `li.scaffold-layout__list-item` selector when locating job cards.
